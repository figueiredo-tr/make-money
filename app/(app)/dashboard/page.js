    ...
    <div className="px-8 py-8 max-w-6xl">
      <header className="mb-8 flex items-end justify-between">
        <div>
          ...
          <h1 className="font-display italic text-3xl text-ink">O Livro</h1>
        </div>
      ...
      {carregando ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <StatCard label="Total emprestado" value={resumo?.total_emprestado} tone="gold" />
            <StatCard label="A receber" value={resumo?.total_a_receber} />
            <StatCard label="Já recebido" value={resumo?.total_recebido} tone="sage" />
            <StatCard label="Empréstimos em aberto" value={resumo?.emprestimos_ativos} format="number" />
            <StatCard label="Empréstimos quitados" value={resumo?.emprestimos_quitados} format="number" tone="sage" />
            <StatCard label="Valor em atraso" value={resumo?.valor_total_atrasado} tone="wine" />
          </div>
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              ...
              {resumo?.parcelas_atrasadas > 0 && (
                <Seal status="atrasado" label={`${resumo.parcelas_atrasadas} parcela(s) atrasada(s)`} />
              )}
            </div>

            {vencimentos.length === 0 ? (
              <p className="text-sm text-faint py-4">Nenhum vencimento nos próximos 7 dias.</p>
            ) : (
                ...
                <tbody>
                  {vencimentos.map((v) => (
                    <tr key={v.parcela_id}>
                      ...
                      <td className="font-mono text-muted">#{v.numero_parcela}</td>
                      <td className="font-mono">
                        {new Date(v.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}
                      </td>
                      <td className="font-mono">
                        {(v.valor_previsto - (v.valor_pago || 0)).toLocaleString('pt-BR', {
                          style: 'currency',
                          currency: 'BRL',
                        })}
                      </td>
