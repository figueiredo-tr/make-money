...
/**
 * Calcula o juros acumulado por atraso.
 * @param {number} valorRestante
 * @param {number} taxaDiaria - % ao dia (se tipo 'percentual') ou R$ ao dia (se tipo 'valor')
 * @param {number} diasAtraso
 * @param {'percentual'|'valor'} tipo
 */
export function calcularJurosAtraso(valorRestante, taxaDiaria, diasAtraso, tipo = 'percentual') {
  if (!taxaDiaria || !diasAtraso || diasAtraso <= 0) return 0;
  if (tipo === 'valor') return arredondar(taxaDiaria * diasAtraso);
  if (!valorRestante) return 0;
