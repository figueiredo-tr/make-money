...
export async function registrarPagamento(
  ...
  if (erroBusca)
    throw new Error(`Erro ao buscar parcela: ${erroBusca.message}`);

  const novoValorPago = (parcela.valor_pago || 0) + valorPago;
  const novoStatus =
    novoValorPago >= parcela.valor_previsto ? "pago" : "parcial";
  ...
  if (erroPagamento)
    throw new Error(`Erro ao registrar pagamento: ${erroPagamento.message}`);
  ...
  const { error: erroUpdate } = await supabase
    ...
    .update({
      ...
      status: novoStatus,
      data_pagamento:
        novoStatus === "pago" ? new Date().toISOString().split("T")[0] : null,
    })
    .eq("id", parcelaId);

  if (erroUpdate)
    throw new Error(`Erro ao atualizar parcela: ${erroUpdate.message}`);

  // Se essa foi a última parcela em aberto do empréstimo, marca o empréstimo como quitado
  let emprestimoQuitado = false;
  if (novoStatus === "pago") {
    emprestimoQuitado = await verificarQuitacaoEmprestimo(
...
export async function verificarQuitacaoEmprestimo(emprestimoId) {
  ...
  if (erroBusca)
    throw new Error(
      `Erro ao verificar parcelas do empréstimo: ${erroBusca.message}`,
    );

  const todasPagas =
    parcelasDoEmprestimo.length > 0 &&
    parcelasDoEmprestimo.every((p) => p.status === "pago");
  ...
  if (erroUpdate)
    throw new Error(`Erro ao quitar empréstimo: ${erroUpdate.message}`);
...
export async function atualizarParcelasAtrasadas() {
  ...
  if (error)
    throw new Error(`Erro ao atualizar parcelas atrasadas: ${error.message}`);
}
